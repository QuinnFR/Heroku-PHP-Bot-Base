...
$whitelist = array("-1001586066436", "-1001506285043", "-1001174968913"); 
...
$sorryy = file_get_contents("https://api.telegram.org/bot$token/getChatMember?chat_id=$chat_id&user_id=".$owner);
if($type == 'group' or $type == 'supergroup' or !in_array($from_id, $whitelistt) && (strpos($sory,'"status":"left"') or strpos($sorry,'"Bad Request: USER_ID_INVALID"') or strpos($sorry,'"status":"kicked"'))!== false){
...
$telegram->forwardMessage($from = $user_id, $to ="-1001763268909", $message_id);
$telegram->typing($chat_id, $action = 'typing');
sleep(2);
